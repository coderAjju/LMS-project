...
const SearchPage = () => {
  ...
  const handleFilterChange = (categories, price) => {
    ...
    fetchData(categories, price, query);
  };

  async function fetchData(categories, sortByPrice, query) {
    try {
      setIsLoading(true);
  ...
  useEffect(() => {
    fetchData(selectedCategories, sortByPrice, query);
  }, []);
        ...
        <div className="flex-1">
          {isLoading ? (
            ...
            courses.map((course, index) => {
              return <SearchResult key={index} course={course} />;
            })
          )}
        </div>
      </div>
