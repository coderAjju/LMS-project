...
const CourseDetailPage = () => {
  ...
  useEffect(()=>{
    async function fetchDetails(){
      try {
        setIsLoading(true);
        const res = await axiosInstance.get(`/api/purchase/${courseId}/detail-with-status`);
    ...
    <div className="mt-24 space-y-5">
      <div className="bg-[#2d2f31] text-white">
        <div className="max-w-7xl mx-auto py-8 px-4 md:px-8 flex flex-col gap-2">
          <h1 className="font-bold text-2xl md:text-3xl">{courseDetail?.courseTitle}</h1>
        ...
        <div className="w-full lg:w-1/2 space-y-5">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{courseDetail?.lectures?.length} lectures</CardDescription>
            </CardHeader>
        ...
        <div className="w-full lg:w-1/3">
            <Card>
              <CardContent>
                ...
                <h1 className="text-lg md:text-xl font-semibold">₹{courseDetail.coursePrice}</h1>
              </CardContent>
              <CardFooter>
                {
                  isCoursePurchased ? (
