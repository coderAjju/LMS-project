...
export const stripeWebhook = async (req, res) => {
    try {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            const course = await CoursePurchase.findOne({ paymentId: session.id }).populate({path:"courseId"});
            ...
            if(course.courseId && course.courseId.lectures.length > 0){
                    await lectureModel.updateMany(
                        {_id:{$in:course.courseId.lectures}},
